//
//  HYIrregularityBasicContentView.swift
//  Himalayan
//
...
class HYIrregularityBasicContentView: HYBouncesContentView {
...
class HYIrregularityContentView: ESTabBarItemContentView {
    override init(frame: CGRect) {
        super.init(frame: frame)
    ...
    override func point(inside point: CGPoint, with event: UIEvent?) -> Bool {
        ...
        return sqrt(pow(imageView.bounds.size.width / 2.0 - p.x, 2) + pow(imageView.bounds.size.height / 2.0 - p.y, 2)) < imageView.bounds.size.width / 2
    }
